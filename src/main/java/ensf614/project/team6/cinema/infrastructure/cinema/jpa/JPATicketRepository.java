...
public interface JPATicketRepository extends JpaRepository<Ticket, Integer> {
    ...
    List<Ticket> getTicketsByMovieIdShowRoomIdAndStartTime(Integer movieId, Integer showRoomId, LocalDateTime startTime);
    ...
    List<LocalDateTime> getMovieStartTimesByMovieIdAndShowRoomId(Integer movieId, Integer showRoomId, LocalDateTime from, LocalDateTime to);
    ...
    @Query(value = "SELECT distinct sr FROM Ticket tic " +
            ...
            "WHERE m.id = :movieId " +
            "AND tic.showTime between :from and :to")
    List<ShowRoom> getShowRoomsPlayingMovie(Integer movieId, LocalDateTime from, LocalDateTime to);
